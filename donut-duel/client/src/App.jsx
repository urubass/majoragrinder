...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'UPDATE_DONUTS': return { ...state, donuts: action.payload, history: [...state.history, action.payload.length].slice(-30) };
...
const Analytics = ({ history }) => {
  const isGrowth = history.length > 1 && history[history.length - 1] > history[history.length - 2];
  const points = history.map((val, i) => `${(i / (history.length - 1)) * 100},${100 - (val * 10)}`).join(' ');
  return (
    <div className={`analytics-glass ${isGrowth ? 'analytics-growth' : ''}`}>
      <h2 className="neon-text">Hospodářský Růst</h2>
      <svg viewBox="0 0 100 100" className="analytics-chart">
        <polyline points={points} />
      </svg>
      <div style={{ fontSize: '0.7rem', color: '#666', marginTop: '10px' }}>KUMULACE KOBLIH V REÁLNÉM ČASE</div>
    </div>
  );
};
...
function App() {
  ...
  useEffect(() => {
    ...
    socketRef.current.on('gameReset', (d) => { dispatch({ type: 'GAME_RESET', payload: d }); displayPlayersRef.current = JSON.parse(JSON.stringify(d.players)); if (state.myId) playerPosRef.current = { x: d.players[state.myId].x, y: d.players[state.myId].y }; });
    ...
    socketRef.current.on('shieldEffect', ({ playerId, active }) => {
      if (active && displayPlayersRef.current[playerId]) spawnParticles(displayPlayersRef.current[playerId].x, displayPlayersRef.current[playerId].y, '#ffffff');
    ...
    socketRef.current.on('chatUpdate', ({ playerId, message }) => {
      if (displayPlayersRef.current[playerId]) spawnChatParticles(displayPlayersRef.current[playerId].x, displayPlayersRef.current[playerId].y, message);
    ...
    socketRef.current.on('eventStart', (ev) => { dispatch({ type: 'EVENT_START', payload: ev }); setShowAnnouncer(true); setTimeout(() => setShowAnnouncer(false), 3000); playSound('boost'); });
    ...
    socketRef.current.on('playerMoved', (p) => { if (p.id !== state.myId) { if (!updateQueueRef.current[p.id]) updateQueueRef.current[p.id] = []; updateQueueRef.current[p.id].push({ x: p.x, y: p.y, ts: Date.now() }); } });
    socketRef.current.on('playerDelta', ({ id, delta }) => { if (id !== state.myId && state.players[id]) { const { dx, dy } = unpackDelta(delta); const lastX = state.players[id].x; const lastY = state.players[id].y; if (!updateQueueRef.current[id]) updateQueueRef.current[id] = []; updateQueueRef.current[id].push({ x: lastX + dx, y: lastY + dy, ts: Date.now() }); } });
  ...
  const spawnParticles = (x, y, color) => { const p = Array.from({ length: 10 }).map(() => ({ id: Math.random(), type: 'pixel', x, y, tx: (Math.random()-0.5)*150, ty: (Math.random()-0.5)*150, color })); dispatch({ type: 'SPAWN_PARTICLES', payload: p }); };
  const spawnChatParticles = (x, y, msg) => { let emoji = '🍩'; if (msg.includes('kampaň')) emoji = '‼️'; if (msg.includes('líp')) emoji = '❤️'; if (msg.includes('kradnú')) emoji = '⛓️'; if (msg.includes('schválena')) emoji = '💰'; const p = Array.from({ length: 5 }).map(() => ({ id: Math.random(), type: 'emoji', emoji, x, y, tx: (Math.random()-0.5)*200, ty: (Math.random()-0.5)*200 })); dispatch({ type: 'SPAWN_PARTICLES', payload: p }); };

  const animate = () => {
    const lerp = (s, e, a) => (1 - a) * s + a * e;
    Object.keys(state.players).forEach(id => {
      const p = state.players[id]; const disp = displayPlayersRef.current[id];
      if (!disp) return;
      if (id === state.myId) { disp.x = lerp(disp.x, p.x, 0.4); disp.y = lerp(disp.y, p.y, 0.4); } else {
        const queue = updateQueueRef.current[id]; if (queue && queue.length > 0) { const target = queue[0]; disp.x = lerp(disp.x, target.x, 0.2); disp.y = lerp(disp.y, target.y, 0.2); if (Math.abs(disp.x - target.x) < 1 && Math.abs(disp.y - target.y) < 1) queue.shift(); }
      }
    });
    document.querySelectorAll('.player').forEach(el => {
      const id = el.getAttribute('data-id'); const p = displayPlayersRef.current[id]; const stateP = state.players[id];
      if (p && stateP) { el.style.left = `${p.x}px`; el.style.top = `${p.y}px`; if (stateP.shieldActive) el.classList.add('shielded'); else el.classList.remove('shielded'); }
    });
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      const p = state.players[state.myId]; if (state.activeEvent?.name === 'AUDIT' && state.activeEvent?.victim === state.myId && !p.shieldActive) return;
      const step = p?.subsidyActive ? 25 : 15;
      let { x, y } = playerPosRef.current; const oldX = x; const oldY = y;
      ...
      if (x !== oldX || y !== oldY) { playerPosRef.current = { x, y }; dispatch({ type: 'UPDATE_PLAYER', payload: { ...p, x, y } }); socketRef.current.emit('moveDelta', packDelta(x - oldX, y - oldY)); }
    };
    ...
    <div className="game-container">
      ...
      <div style={{ display: 'flex', gap: '20px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div className="shop-glass"><h2 className="neon-text">Shop</h2><button className="btn-premium-shop" onClick={() => socketRef.current.emit('buyCertificate', 'boost')} disabled={state.players[state.myId]?.score < 20}>BOOST (20🍩)</button><button className="btn-premium-shop" style={{ marginTop: '10px' }} onClick={() => socketRef.current.emit('buyCertificate', 'shield')} disabled={state.players[state.myId]?.score < 30}>ŠTÍT (30🍩)</button></div>
        ...
        <div className={`arena ${state.donuts.length < 3 ? 'crisis' : ''} ${state.activeEvent?.name === 'EET_BONUS' ? 'eet-active' : ''}`} style={{ width: state.arenaSize, height: state.arenaSize }}>
          <div className={`overlay overlay-audit ${state.activeEvent?.name === 'AUDIT' ? 'active-overlay' : ''}`} /><div className={`overlay overlay-eet ${state.activeEvent?.name === 'EET_BONUS' ? 'active-overlay' : ''}`} />
          ...
          {state.particles.map(p => p.type === 'pixel' ? (<div key={p.id} className="particle-burst" style={{ left: p.x, top: p.y, backgroundColor: p.color, '--tx': `${p.tx}px`, '--ty': `${p.ty}px` }} />) : (<div key={p.id} className="particle-emoji" style={{ left: p.x, top: p.y, '--tx': `${p.tx}px`, '--ty': `${p.ty}px` }}>{p.emoji}</div>))}
          {Object.values(state.players).map(p => (<div key={p.id} data-id={p.id} className={`player ${p.subsidyActive ? 'active-subsidy' : ''} ${state.activeEvent?.name === 'AUDIT' && state.activeEvent?.victim === p.id && !p.shieldActive ? 'frozen' : ''}`} style={{ backgroundColor: p.color, color: p.color, border: p.id === state.myId ? '2px solid white' : 'none' }}>{p.chat && <div className="chat-bubble">{p.chat}</div>}{p.shieldActive && <div className="shield-icon">🛡️</div>}<div className="player-label">{p.id === state.myId ? 'VY' : 'SÚPER'} ({p.score})</div></div>))}
        </div>
      </div>
    </div>
  );
